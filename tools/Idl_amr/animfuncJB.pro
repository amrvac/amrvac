...
;---------------------------------------------------------------------------
; 
; 15-5-2002 JB started adding 'rhd' physics
;
;===========================================================================

animfdebug=0
if animfdebug then print,'*** Start animfunc with: ',func
...
case physics of
   ...
   'rhd11':begin
      d=w(*,0) & s1=w(*,1) & tau=w(*,2)  
      lfac=w(*,3) & p=w(*,4)
   end
   'rhd12':begin
      d=w(*,0) & s1=w(*,1) & s2=w(*,2) & tau=w(*,3) 
      lfac=w(*,4) & p=w(*,5)
   end
   'rhd13':begin
      d=w(*,0) & s1=w(*,1) & s2=w(*,2) & s3=w(*,3) & tau=w(*,4)
      lfac=w(*,5) & p=w(*,6)
   end
   'rmhd13':begin
      d=w(*,0) & s1=w(*,1) & s2=w(*,2) & s3=w(*,3) & tau=w(*,4)
      b1=w(*,5) & b2=w(*,6) & b3=w(*,7) & lfac=w(*,8) & xi=w(*,9)
   end
   'rhd22':begin
      d=w(*,*,0) & s1=w(*,*,1) & s2=w(*,*,2) & tau=w(*,*,3)
      lfac=w(*,*,4) & p=w(*,*,5)
   ...
   'rhd33':begin
      d=w(*,*,*,0) & s1=w(*,*,*,1) & s2=w(*,*,*,2) & s3=w(*,*,*,3)
      tau=w(*,*,*,4) & lfac=w(*,*,*,5) & p=w(*,*,*,6)
   end
   'mhdiso11':begin
...
if n_elements(m1) gt 0 then case ndir of 
    ...
    3: mm=m1^2+m2^2+m3^2
endcase
if n_elements(s1) gt 0 then case ndir of 
    1: ss=s1^2
    2: ss=s1^2+s2^2
    3: ss=s1^2+s2^2+s3^2
endcase
if n_elements(b1) gt 0 then case ndir of 
    ...
    3: bb=b1^2+b2^2+b3^2
endcase
if n_elements(b1) gt 0 and n_elements(s1) gt 0 then case ndir of 
    1: sdotb=b1*s1
    2: sdotb=b1*s1+b2*s2
    3: sdotb=b1*s1+b2*s2+b3*s3
endcase
...
if  phys eq 'rmhd' then begin ;===RK: Relativistic MHD

  rho=d/lfac
  p=(gamma-1.0d0)*(xi/lfac^2-rho)/gamma
  case ndir of 
    1: begin
         v1=(s1+sdotb*b1/xi)/(xi+bb)
         vdotb=v1*b1
       end
    2: begin 
         v1=(s1+sdotb*b1/xi)/(xi+bb)
         v2=(s2+sdotb*b2/xi)/(xi+bb)
         vdotb=v1*b1+v2*b2
       end
    3: begin
         v1=(s1+sdotb*b1/xi)/(xi+bb)
         v2=(s2+sdotb*b2/xi)/(xi+bb)
         v3=(s3+sdotb*b3/xi)/(xi+bb)
         vdotb=v1*b1+v2*b2+v3*b3
       end
  endcase
  case 1 of
    f eq 'p'                          : result=p
    f eq 'logp'                       : result=alog10(p)
    f eq 'ptot'                       : result=p+0.5d0*(bb/lfac^2+vdotb^2)
    f eq 'logptot'                    : result=alog10(p+0.5d0*(bb/lfac^2+vdotb^2))
    f eq 'rho'                        : result=rho
    f eq 'rho25'                      : result=rho/25.0
    f eq 'logrho'                     : result=alog10(rho)
    f eq 'p1000'                      : result=p/1000.0
    ; Velocities
    f eq 'vx' or f eq 'v1' or f eq 'v': result=v1
    f eq 'vy' or f eq 'v2'            : result=v2
    f eq 'vz' or f eq 'v3'            : result=v3
    f eq 'E'                          : result=tau+D
    f eq 'ent'                        : result=rho+gamma*p/(gamma-1.0d0)
    f eq 'Sent'                       : result=p*rho^(-gamma)
  endcase
endif else if  phys eq 'rhd' then begin ;===JB: Relativistic hydro
  ...
  case 1 of
    ...
    f eq 'p1000'                      : result=p/1000.0
    f eq 'logp'                       : result=alog10(p)
    f eq 'rhg2'                       : result=rhohlfac2
    ; Velocities
    f eq 'vx' or f eq 'v1' or f eq 'v': result=v1
    f eq 'vy' or f eq 'v2'            : result=v2
    f eq 'vz' or f eq 'v3'            : result=v3
    f eq 'csound'                     : result=sqrt(gamma*p*lfac^2/rhohlfac2)
    ; internal energy, enthalpy
    f eq 'e'                          : result=rhohlfac2/(lfac*lfac)-1.0d0-p/rho
    f eq 'eps'                        : result=p/(rho*(gamma-1.0d0))
    f eq 'h'                          : result= rhohlfac2/(rho*lfac^2)
  endcase
endif else begin    ; all other physics
  case 1 of
    ; Velocities
    f eq 'vx' or f eq 'v1' or f eq 'v': result=m1/rho
    ...
    f eq 'p' or f eq 'pth' or f eq 'pbeta' or f eq 'logp' or f eq 'logT' or f eq 'T' or f eq 's' or $
    f eq 'eps' or f eq 'csound' or strmid(f,0,4) eq 'mach' or $
    strmid(f,0,5) eq 'cfast' or strmid(f,0,5) eq 'cslow' or $
    strmid(f,0,5) eq 'Mfast' or strmid(f,0,5) eq 'Mslow' or f eq 'pram1' or f eq 'logpram1' $
    :begin
       ...
       case f of
           ...
           'pram1'  : result=m1^2/rho
           'logpram1'  : result=alog10(m1^2/rho)
    ...
    ; Powells source terms in 2 and 2.5D MHD
