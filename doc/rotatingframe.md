...
**NOTE**: The user should be aware that the fictitious forces are **not** added in an angular momentum conserving fashion in cylindrical and spherical geometries (in Cartesian geometry it will never be the case). An angular momentum conserving method incorporates the azimuthal component of the Coriolis force directly into the azimuthal flux computation within the conservative governing equations. Such angular momentum conserving formulation may be important for some applications, see for example [Kley (1998)](https://ui.adsabs.harvard.edu/abs/1998A%26A...338L..37K/abstract). The present implementation in MPI-AMRVAC adds all fictitious force components displayed below as physical source terms to the conservative governing equations.
