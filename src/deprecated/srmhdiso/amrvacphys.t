!#############################################################################
! module amrvacphys - srmhd  version april 2013 -- DEsigned by Fabien Casse
...
if(eqpar(gamma_)<=zero .or. eqpar(gamma_)==one) then
  call mpistop("non-polytropic case or gamma=1 not implemented")
endif


govergminone =eqpar(gamma_)/(eqpar(gamma_)-one)
! --------------- constant total pressure Ptot = cst ----------
minp   = max(zero,eqpar(csound2_)*smallrho)
...
if(.not.strictzero)limitvalue=smalldouble
...
double precision, dimension(ixG^T):: sqrV,sqrU,sqrB,VdotB,rhoh,xi
!-----------------------------------------------------------------------------
...
where(.not.patchw(ixO^S))
   w(ixO^S,xi_)=w(ixO^S,lfac_)*w(ixO^S,lfac_) &
                 *w(ixO^S,rho_)*(one+govergminone*eqpar(csound2_))
...
where(.not.patchw(ixO^S))
    w(ixO^S,tau_)=w(ixO^S,xi_) - eqpar(csound2_)*&
         w(ixO^S,d_)/w(ixO^S,lfac_) - w(ixO^S,d_) +& 
...
where(.not.patchw(ixO^S))
   w(ixO^S,xi_)=w(ixO^S,lfac_)*w(ixO^S,lfac_) &
                 *(w(ixO^S,rho_)+govergminone*w(ixO^S,rho_)*eqpar(csound2_)) 
   w(ixO^S,d_) =w(ixO^S,rho_)*w(ixO^S,lfac_)
   ! recycle sqrU array for storing temporary positive 
   ! array for use in energy variable
   sqrU(ixO^S)=sqrB(ixO^S)*sqrV(ixO^S)-VdotB(ixO^S)**2.0d0
...
! instead of E use tau= E - D
where(.not.patchw(ixO^S))
    w(ixO^S,tau_)=w(ixO^S,xi_) - eqpar(csound2_)*w(ixO^S,d_)/&
         w(ixO^S,lfac_) - w(ixO^S,d_) +& 
         half*(sqrB(ixO^S) + sqrU(ixO^S))
endwhere
...
double precision, intent(in)       :: x(ixI^S,1:ndim)
double precision, dimension(ixG^T) :: SdotB, sqrB, sqrBini
...
!----------- Here we prescribe a constant entropy, ie P = cst*rho**gamma
w(ixO^S,rho_)=w(ixO^S,d_)/w(ixO^S,lfac_)
w(ixO^S,pp_)=eqpar(csound2_)*w(ixO^S,rho_)
...
if(useprimitive)then
   ...
   if (useprimitiveRel) then
      ...
      ptot(ixI^S) = w(ixI^S,pp_)+&
           half*(({^C&w(ixI^S,v^C_)*w(ixI^S,b^C_)+})  &
                  +({^C&w(ixI^S,b^C_)**2.0d0+})/w(ixI^S,lfac_)**2.0d0)
   end if
...
double precision,  dimension(ixG^T,1:nw) :: wtemp
...
bxzero : if(any(Domain(ixO^S,3)))then
 if(needcmin)then
  ...
  where(Domain(ixO^S,3))
   ...
   cmax(ixO^S)= min(max((B(ixO^S)+dsqrt(max(zero,B(ixO^S)**2.0d0-A(ixO^S)*C(ixO^S))))&
              /A(ixO^S),zero), one)
    ...
    {if (iw==s^C_) then
        ...
        if (idims==^C) then !!! add ptot
           ptot(ixO^S) = half*(VdotB(ixO^S)**2.0d0  & 
                       ...
                       + eqpar(csound2_)*w(ixO^S,d_)/w(ixO^S,lfac_)
...
double precision, dimension(ixG^T):: VdotB, sqrB, ptot, sini
!-----------------------------------------------------------------------------
    ...
    {if (iw==s^C_) then
        f(ixO^S,iw)=-w(ixO^S,b0_+idims)*(VdotB(ixO^S)*&
                   ...
                   (w(ixO^S,xi_)+sqrB(ixO^S)) + &
                    w(ixO^S,b^C_)/w(ixO^S,lfac_)**2.0d0)
        if (idims==^C) then !!! add ptot
           ptot(ixO^S)= eqpar(csound2_)*w(ixO^S,d_)/w(ixO^S,lfac_)+ &
                half*(VdotB(ixO^S)**2.0d0+ sqrB(ixO^S)/(w(ixO^S,lfac_)**2.0d0))       
...
!=============================================================================
subroutine funcdlor(lfac,F,dF,xi,tau,d,ssqr,bsqr,sdotb,ierror)

use mod_global_parameters

double precision, intent(in)  :: lfac,d,ssqr,bsqr,sdotb
double precision, intent(out) :: F,dF,xi,tau
integer, intent(inout)        :: ierror
  
double precision  :: dlfac,sb2
double precision  :: vsqr,g1, co,vdotb
!-----------------------------------------------------------------------------

sb2 = sdotb*sdotb
vsqr = ((ssqr + (xi*two*sb2 + bsqr*sb2)/(xi*xi))/((xi+bsqr)*(xi+bsqr)))
g1 = (eqpar(gamma_)-one)/eqpar(gamma_)
co= one + govergminone*eqpar(csound2_)
xi = lfac*d*co
vdotb=sdotb/xi
tau=xi - eqpar(csound2_)*d/lfac -d + bsqr*(one-half/lfac**two)&
        -half*(vdotb)**two

F = (d*co)**2.d0*lfac**4.D0 + two*bsqr*d*co*lfac**3.d0 + (bsqr**two -&
     (d*co)**two -ssqr)*lfac**two - two*lfac*(bsqr*d*co+sb2/(d*co)) -&
     bsqr**two - sb2*bsqr/(d*co)**two

dF = 4.d0*(d*co)**2.d0*lfac**3.d0 + 6.d0*bsqr*d*co*lfac**two+(bsqr**two-&
     (d*co)**two -ssqr)*lfac*two - two*(bsqr*d*co+sb2/(d*co))

ierror=0

end subroutine funcdlor


!@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ LORENTZ FACTOR VERSION
subroutine con2prim(lfac,xi,d,s^C,tau,b^C,ierror)

! (D,S,B) --> compute auxiliaries lfac, xi and tau
...
double precision:: lfac1,lfac2,xii,xih,xil,dxi,f,df
double precision:: temp,vsqr,fl,fh,lfacl,lfach,lfaci
double precision:: er,er1,lfacprev,dplus,lfacmax
double precision:: fv,dfv,v^C,pressure,lfac1min,lfac1max
double precision:: lastxiok,lastlfacok,lastf,vdotb
...
! ierror=1 : error on entry: must have D>=0
...
!  xi = eqpar(gamma_) *(tau - half * bsqr)+d
   xi = d*(one + eqpar(csound2_)*govergminone)
  lfac=one
  vdotb=zero
  tau=xi - eqpar(csound2_)*d/lfac -d + bsqr*(one-half/lfac**two)&
        -half*(vdotb)**two
...
if(bsqr<=limitvalue)then
  ! -- For isothermal RHD the solution is trivial
   lfac=dsqrt(ssqr+(d*(one + eqpar(csound2_)*govergminone))**two)/&
        (d*(one + eqpar(csound2_)*govergminone))
   xi=lfac*d*(one + eqpar(csound2_)*govergminone)
   vdotb=sdotb/xi
   tau=xi - eqpar(csound2_)*d/lfac -d + bsqr*(one-half/lfac**two)&
        -half*(vdotb)**two
  return 
...
dplus=d*(one+eqpar(csound2_)*govergminone)!+eqpar(gamma_)*minp/(eqpar(gamma_)-one)

lfac1=one
!@@@@@@@@@@@@@@@@@@@ Computing the lower bound on LFAC
   lfac1min=lfac1
   lfac1max=lfac1
   fh=-1.d0
do while(fh.le.zero)
   lfac1max=dexp(1.d0)*lfac1max
   call funcdlor(lfac1max,fh,df,xih,tau,d,ssqr,bsqr,sdotb,ierror)
enddo
lfac2=lfac1max

LoopXimin:  do ni = 1,maxitnr
    lfac1=(lfac1min + lfac1max)*half
    call funcdlor(lfac1,fl,df,xil,tau,d,ssqr,bsqr,sdotb,ierror)
        if (fl.lt.zero) then
            if(df.le.zero) then        
             lfac1min=lfac1
            endif
...
if(niiter==maxitnr)then
 ...
 print *,' Con2prim could not find value of lower bound for lfac'
 print *,'lfac1=',lfac1,'dplus=',dplus,' d=',d,'lfac1 et lfac2',lfac1min,lfac1max
 print *,'d=',d,bsqr,ssqr
 ierror=2
 return
endif
...
if(fl>zero)then
  ...
  print *,'lfac1=',lfac1,' vs dplus=',dplus,'dplus*lfacl=',dplus*lfacl&
...
call funcdlor(lfac2,fh,df,xih,tau,d,ssqr,bsqr,sdotb,ierror)
if(fh<zero)then
  print *,'warning: higher bound negative f(xi) value!'
  print *,'lfac2=',lfac2,' vs dplus=',dplus,'dplus*lfacl=',dplus*lfacl&
         ,'lfacl=',lfacl,'lfacmax=',lfacmax
  print *,'fl=',fl,' niiter=',niiter,' vsqr(dplus)=',vsqr
  ierror=5
  return
...
if(fl==zero)then
  lfaci=lfac1
  xii=xil
  finished=(xil>=dplus*lfacl.and.lfac1<=lfacmax)
endif

if(fh==zero)then
  lfaci=lfac2
  xii=xih
  finished=(xih>=dplus*lfach.and.lfac2<=lfacmax)
endif
...
lfacl=lfac1
lfach=lfac2
lfaci=half*(lfach+lfacl)    !Initialize the guess for rootfinder
call funcdlor(lfaci,f,df,xii,tau,d,ssqr,bsqr,sdotb,ierror)
!if(ierror /=0)return
...
!      if(ierror /=0)return
      ...
      if(f*df==zero) then
         if(f==zero) then
            tests=(lfaci>=1.d0.and.lfaci<=lfacmax)
            if(tests)then
        ...
        if(f*df>zero)then
          ...
          ! restrict to left
          lfaci=max(lfaci,lfacl)
        else
         ! xi-iterate increased
         ! restrict to right
          lfaci=min(lfaci,lfach)
        endif
      endif
      er=dabs(f/df)/lfaci
      if((er<tolernr*er1).or.(dabs(f/df)<absaccnr))then
            call funcdlor(lfaci,f,df,xii,tau,d,ssqr,bsqr,sdotb,ierror) 
            tests=(xii>=d*lfaci.and.lfaci<=lfacmax)
            if(tests)then
...
{^NOONED
! s[b1]=cot(theta)*psi/r
         tmp(ixO^S)=wCT(ixO^S,psi_)*dcos(x(ixO^S,2))/dsin(x(ixO^S,2))
!         w(ixO^S,iw)=w(ixO^S,iw)+qdt*wCT(ixO^S,psi_) &
!                     *(mygeo%surfaceC2(ixO^S)-mygeo%surfaceC2(h2x^S)) &
!                     /mygeo%dvolume(ixO^S)
}
{^NOONEC
            tmp(ixO^S)=tmp(ixO^S) + (wCT(ixO^S,s1_)*wCT(ixO^S,b2_) &
...
{^IFTHREEC
         ...
         if(iw==s1_{^NOONEC.or.iw==s2_.or.iw==b2_}{^IFTHREEC .or.iw==b3_ &
               .or.(iw==s3_.and..not.angmomfix)}{#IFDEF GLM .or. iw==b1_}) &
            w(ixO^S,iw)=w(ixO^S,iw)+qdt*tmp(ixO^S)/x(ixO^S,1)
      end do
end select
...
!if (typedivbfix=='powel' .or. typedivbfix=='janhunen' .or. typedivbfix=='linde') &
