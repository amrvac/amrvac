...
contains
  ...
  subroutine srmhd_average(wL,wR,x,ix^L,idim,wroe,workroe)
  ...
  subroutine average2(wL,wR,x,ix^L,idim,wroe,cfast,cslow,afast,aslow,csound2,dpressure, &
    ...
    call srmhd_get_pthermal(wL,x,ixG^LL,ix^L,afast)
    call srmhd_get_pthermal(wR,x,ixG^LL,ix^L,aslow)
    ...
    ! get csound**2
    if(srmhd_energy) then
      csound2(ix^S)=srmhd_gamma*wroe(ix^S,p_)/wroe(ix^S,rho_)
    else
      csound2(ix^S)=srmhd_gamma*srmhd_adiab*wroe(ix^S,rho_)**(srmhd_gamma-1.0d0)
    end if
  ...
  subroutine srmhd_get_eigenjump(wL,wR,wroe,x,ix^L,il,idim,smalla,a,jump,workroe)
    use mod_global_parameters
  ...
  subroutine geteigenjump2(wL,wR,wroe,x,ix^L,il,idim,smalla,a,jump, &
    ...
    case('harten','powell', 'ratio')
       ...
       case(fastRW_)
          ...
          call srmhd_get_csound2(wL,x,ixG^LL,ix^L,rhohL,cs2L)
          call srmhd_get_csound2(wR,x,ixG^LL,ix^L,rhohR,cs2R)
  ...
  subroutine srmhd_rtimes(q,w,ix^L,iw,il,idim,rq,workroe)
    use mod_global_parameters
    ...
    if(iw == rho_) then
      ...
      if(il==fastRW_)then
        ...
        v2a2(ix^S)=0.5d0*sum(wroe(ix^S,mom(:))**2,dim=ndim+1)+ &
             (2.0d0-srmhd_gamma)/(srmhd_gamma-1.0d0)*csound2(ix^S)
