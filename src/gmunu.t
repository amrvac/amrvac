...
program gmunu
  ...
  if (restart_from_file /= undefined) then
     ...
     if (associated(usr_restart_process)) &
        call usr_restart_process(it,global_time)
...
contains
  ...
  subroutine timeintegration()
    ...
    time_evol : do
       ...
       ! Convert ps into primitive state,
       ! this is bacause the calculation of dt needs it,
       ! Also, we mostly output prim instead of cons,
       ! In case cons is needed, just do P2C. 
       ! P2C is analytic thus much faster then C2P
       ! Note that ghost cells is also included
       do iigrid=1,igridstail; igrid=igrids(iigrid);
         if (.not.ps(igrid)%is_prim) &
           call phys_to_primitive(ixG^LL,ixG^LL,ps(igrid))
       end do
       ...
       if (any(save_file)) then
         if(associated(usr_modify_output)) then
           ...
           do iigrid=1,igridstail; igrid=igrids(iigrid);
             ...
             block=>ps(igrid)
             call usr_modify_output(ixG^LL,ixM^LL,global_time,ps(igrid))
           end do
         end if
         time_write=0.d0
         do ifile=nfile,1,-1
       ...
       ! fixme: rearrange the order so that the time calculation make more sense
       ...
       if (crashall) then
         ...
         call saveamrfile(1)
         call saveamrfile(2)
         if(mype==0) write(*,*) "Error: Error encountered, run crash."
