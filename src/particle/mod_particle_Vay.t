...
!        particle(ipart)%self%u = uplus + emom + radmom
        ...
        !!! VAY MOVER
        call cross(particle(ipart)%self%u,b,tmp)
        uhalf(1:ndir) = particle(ipart)%self%u(1:ndir) + q * dt_p /(2.0d0 * m * const_c) * (e(1:ndir) + tmp(1:ndir)/lfac)
        ...
        lfac = sqrt((sigma + sqrt(sigma**2 + 4.d0 * dot_product(tau,tau) + dot_product(ustar,ustar))) / 2.d0)
