...
contains
  ...
  subroutine advect_create_particles()
    ...
    if (mype==0) then
      if (.not. associated(usr_create_particles)) then
        ...
        do n=1, num_particles
          x(1,n) = xprobmin1 + rrd(n,1) * (xprobmax1 - xprobmin1)
          x(2,n) = xprobmin2 + rrd(n,2) * (xprobmax2 - xprobmin2)
          x(3,n) = xprobmin3 + rrd(n,3) * (xprobmax3 - xprobmin3)
        end do
      else
  ...
  subroutine advect_fill_gridvars
    ...
    do iigrid=1,igridstail; igrid=igrids(iigrid);
      ...
      gridvars(igrid)%w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,ixGlo3:ixGhi3,&
         rhop) = w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,ixGlo3:ixGhi3,iw_rho)
    end do
  ...
  function advect_get_particle_dt(partp, end_time) result(dt_p)
    ...
    do idims=1,ndim
      dtdims(idims)=minval(ps(partp%igrid)%ds(ixMlo1:ixMhi1,ixMlo2:ixMhi2,&
         ixMlo3:ixMhi3,idims))/(max(abs(v(idims)),smalldouble))
    end do
