...
contains
  ...
  subroutine mg_setup_multigrid()
    ...
    if (any([ block_nx1,block_nx2,block_nx3 ] /= block_nx1)) error stop "Multigrid requires all block_nx to be equal"
  ...
  subroutine mg_copy_to_tree(iw_from, iw_to, restrict, restrict_gc, factor,&
    ...
    integer                        :: nc, lvl, ilo1,ilo2,ilo3, ihi1,ihi2,ihi3
    ...
    ilo1=ixMlo1-1;ilo2=ixMlo2-1;ilo3=ixMlo3-1;
    ihi1=ixMhi1+1;ihi2=ixMhi2+1;ihi3=ixMhi3+1;
    ...
    do iigrid = 1, igridstail
       ...
       if (present(state_from)) then
          mg%boxes(id)%cc(0:nc+1,0:nc+1,0:nc+1,&
              iw_to) = fac * state_from(igrid)%w(ilo1:ihi1,ilo2:ihi2,ilo3:ihi3,&
              iw_from)
       else
          mg%boxes(id)%cc(0:nc+1,0:nc+1,0:nc+1,&
              iw_to) = fac * ps(igrid)%w(ilo1:ihi1,ilo2:ihi2,ilo3:ihi3,&
              iw_from)
       end if
    end do
  ...
  subroutine mg_copy_from_tree(iw_from, iw_to, state_to)
    ...
    do iigrid = 1, igridstail
       ...
       if (present(state_to)) then
          state_to(igrid)%w(ixMlo1:ixMhi1,ixMlo2:ixMhi2,ixMlo3:ixMhi3,&
              iw_to) = mg%boxes(id)%cc(1:nc,1:nc,1:nc, iw_from)
       else
  ...
  subroutine mg_copy_from_tree_gc(iw_from, iw_to, state_to)
    ...
    do iigrid = 1, igridstail
       ...
       if (present(state_to)) then
          state_to(igrid)%w(ilo1:ihi1,ilo2:ihi2,ilo3:ihi3,&
              iw_to) = mg%boxes(id)%cc(0:nc+1,0:nc+1,0:nc+1, iw_from)
       else
          ps(igrid)%w(ilo1:ihi1,ilo2:ihi2,ilo3:ihi3,&
              iw_to) = mg%boxes(id)%cc(0:nc+1,0:nc+1,0:nc+1, iw_from)
       end if
    end do
  ...
  subroutine mg_tree_from_amrvac(mg)
    ...
    call mg_build_rectangle(mg, [ domain_nx1,domain_nx2,domain_nx3 ],&
        block_nx1, dx(:,1), [ xprobmin1,xprobmin2,xprobmin3 ], periodB,&
        n_finer)
