...
module mod_fft
  ...
  public :: fft_1d
  public :: fft_2d
  public :: fft_2d_real_imag
  public :: fft_raw
  public :: fft_size_supported
  public :: fft_next_supported
  public :: fft_factorization

contains

  !> Return whether the legacy mixed-radix workspace can handle a transform.
  logical function fft_size_supported(n)
    integer, intent(in) :: n
    integer :: m,p,nleft,nfactors,square_free

    if(n<1) then
      fft_size_supported=.false.
      return
    end if
    ...
    do while(p*p<=nleft)
      if(mod(nleft,p)==0) then
        ...
        square_free=square_free*p
        do while(mod(nleft,p)==0)
          nleft=nleft/p
          nfactors=nfactors+1
        end do
      end if
    ...
    if(nleft>1) then
      ...
      square_free=square_free*nleft
      nfactors=nfactors+1
    end if

    ! The embedded algorithm stores at most 11 factors and 209 permutation
    ! entries. Repeated powers of two are combined internally, so this check is
    ! deliberately conservative only for the general prime-factor path.
    m=n
    nfactors=0
    do p=2,23
      do while(mod(m,p)==0)
    ...
    fft_size_supported=(m==1 .and. nfactors<=11 .and. square_free<=210)
  end function fft_size_supported

  integer function fft_next_supported(n)
    integer, intent(in) :: n

    fft_next_supported=max(1,n)
    do while(.not.fft_size_supported(fft_next_supported))
      fft_next_supported=fft_next_supported+1
    end do
  end function fft_next_supported

  !> Human-readable prime factorization used in unsupported-size diagnostics.
  function fft_factorization(n) result(description)
    integer, intent(in) :: n
    character(len=128) :: description
    character(len=24) :: item
    integer :: left,p,power,used

    description=''
    if(n<1) then
      write(description,'(i0)') n
      return
    end if
    ...
    do while(p*p<=left)
      ...
      if(power>0) then
        if(power==1) then
          ...
          write(item,'(i0,a,i0)') p,'^',power
        end if
        if(used>0) description=trim(description)//' * '
        description=trim(description)//trim(item)
        used=used+1
      end if
      if(p==2) then
        p=3
    ...
    if(len_trim(description)==0) description='1'
  end function fft_factorization

  !> In-place complex one-dimensional FFT. The inverse is normalized.
  subroutine fft_1d(data,inverse)
    ...
    n=size(data)
    if(.not.fft_size_supported(n)) error stop 'fft_1d: unsupported transform size'
    allocate(ar(n),ai(n))
    ar=dble(data)
    ai=dimag(data)
    if(inverse) then
    ...
    call fft_raw(ar,ai,n,n,n,isign)
    data=dcmplx(ar,ai)
    if(inverse) data=data/dble(n)
    deallocate(ar,ai)
  end subroutine fft_1d

  !> In-place complex two-dimensional FFT. The inverse is normalized.
  subroutine fft_2d(data,inverse)
    double complex, intent(inout) :: data(:,:)
    logical, intent(in) :: inverse
    double precision, allocatable :: ar(:),ai(:)
    integer :: n1,n2,ntot,isign

    n1=size(data,1)
    n2=size(data,2)
    if(.not.fft_size_supported(n1) .or. .not.fft_size_supported(n2)) &
       error stop 'fft_2d: unsupported transform size'

    ntot=n1*n2
    allocate(ar(ntot),ai(ntot))
    ar=reshape(dble(data),[ntot])
    ai=reshape(dimag(data),[ntot])
    if(inverse) then
      isign=1
    else
      isign=-1
    end if
    call fft_raw(ar,ai,ntot,n1,n1,isign)
    call fft_raw(ar,ai,ntot,n2,ntot,isign)
    data=reshape(dcmplx(ar,ai),shape(data))
    if(inverse) data=data/dble(ntot)
    deallocate(ar,ai)
  end subroutine fft_2d

  !> In-place two-dimensional FFT on split real/imaginary storage. This avoids
  !> allocation and copying in repeated plane transforms.
  subroutine fft_2d_real_imag(ar,ai,inverse)
    double precision, intent(inout) :: ar(:,:),ai(:,:)
    logical, intent(in) :: inverse
    integer :: n1,n2,ntot,isign

    n1=size(ar,1)
    n2=size(ar,2)
    if(any(shape(ai)/=shape(ar))) error stop 'fft_2d_real_imag: shape mismatch'
    if(.not.fft_size_supported(n1) .or. .not.fft_size_supported(n2)) &
       error stop 'fft_2d_real_imag: unsupported transform size'
  ...
  end subroutine fft_2d_real_imag

  subroutine fft_raw(a,b,ntot,n,nspan,isn)
