...
module mod_supertimestepping
  ...
  ! define input parameters from parameter file
 ...
 !end subroutines pointers in this module 
...
111    close(unitpar)
  ...
  !> subroutine wich added programatically a term to be calculated usong STS
  !> Params: 
  !> sts_getdt function which calculates the explicit timestep for this term
  !> sts_set_sources subroutine wich sets the source term
  !> startVar, endVar indices of the start and end of the variables that must be exchanged
  !> indexChangeStart, indexChangeN arrays specify the variables that are updated
  !>    with the STS scheme: the first array  contians the start indices and the second the length
  !> indexChangeFixC array specify if the variables defined by  indexChangeStart, indexChangeN should be fix_conserved
  !>   (the fluxes shoud be stored with store_flux_var  by the subroutine sts_set_sources)
  !> Note that  indexChangeStart, indexChangeN and indexChangeFixC should have equal length
...
!    type_recv_p=>type_recv_p_f
  ...
  !> Sets the hooks called before the first cycle and after the last cycle in the STS update
  ...
  !> Sets the hook of error handling in the STS update. This methios is called before updating the BC.
  ...
  !> This sets the expliict dt and calculates the number of cycles for each of the terms implemented with STS.
...
!!! STS2 methods end
  ...
  ! Turlough Downes 2006,2007
    ...
    do while(associated(temp))
      ...
      !!In the first step bc are already called
