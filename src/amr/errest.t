...
subroutine lohner_grid(igrid)
  ...
  do iflag=1,nw+1
     ...
     if (iflag > nw) then
        if (.not. associated(usr_var_for_errest)) then
           ...
           call usr_var_for_errest(ixG^LL,ixG^LL,iflag,ps(igrid),tmp1)
        end if
     end if
  ...
  {do ix^DB=ixMlo^DB,ixMhi^DB\}
     ...
     if (associated(usr_refine_threshold)) then
        ...
        xtol(1:ndim) = ps(igrid)%mesh%x(ix^D,1:ndim)
...
subroutine forcedrefine_grid(igrid)
  ...
  if (associated(usr_refine_grid)) then
     call usr_refine_grid(igrid,level,ixG^LL,ixM^LL,qt,ps(igrid), &
...
subroutine forcedrefine_grid_io(igrid)
  use mod_forest, only: coarsen, refine
  use mod_global_parameters
