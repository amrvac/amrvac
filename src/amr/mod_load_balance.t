...
contains
  ...
  subroutine load_balance
    ...
    !> MPI recv send metric variables for AMR
    ...
    ! fixme: now, the metric and hydro variables are treated seprately.
    ! They actually have the same array size and of course able to be sent at
    ! the same time. However, this requires a dynamical buffer. Not sure which
    ! is the best way to do so. Need to check.
    ...
    contains
      ...
      subroutine lb_recv
        ...
        call MPI_IRECV(metric(recv_igrid)%vars,1,type_block_io_m,send_ipe,itag, &
      ...
      subroutine lb_send
        ...
        call MPI_ISEND(ps(send_igrid)%w,1,type_block_io,recv_ipe,itag, &
                       icomm,sendrequest(isend),ierrmpi)
        itag=itag+max_blocks
        call MPI_ISEND(metric(send_igrid)%vars,1,type_block_io_m,recv_ipe,itag, &
