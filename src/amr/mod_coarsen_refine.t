...
module mod_coarsen_refine
  ...
  !> MPI recv send variables for metric variables AMR
  integer, dimension(:), allocatable :: recvrequest_m, sendrequest_m
  integer, dimension(:,:), allocatable :: recvstatus_m, sendstatus_m
...
contains
  ...
  subroutine amr_coarsen_refine
    ...
    allocate(recvstatus_m(MPI_STATUS_SIZE,max_blocks*^ND),recvrequest_m(max_blocks*^ND), &
           sendstatus_m(MPI_STATUS_SIZE,max_blocks*^ND),sendrequest_m(max_blocks*^ND))
    recvrequest_m=MPI_REQUEST_NULL
    sendrequest_m=MPI_REQUEST_NULL
    ...
    if (irecv>0) then
      ...
      call MPI_WAITALL(irecv,recvrequest_m,recvstatus_m,ierrmpi)
    ...
    if (isend>0) then
      ...
      call MPI_WAITALL(isend,sendrequest_m,sendstatus_m,ierrmpi)
    ...
    deallocate(recvstatus_m,recvrequest_m,sendstatus_m,sendrequest_m)
    ...
    if ( fix_small_values ) then
       ...
       do iigrid=1,igridstail; igrid=igrids(iigrid);
          call phys_handle_small_values(ps(igrid),ixG^LL,ixM^LL, 'amr_coarsen_refine')
       end do
       !$OMP END PARALLEL DO
    end if
    ...
    if (check_NaN_values) then
       !$OMP PARALLEL DO PRIVATE(igrid)
       do iigrid=1,igridstail; igrid=igrids(iigrid);
          call w_NaN_checker(ixG^LL,ixM^LL,ps(igrid)%w(ixG^T,1:nw),ps(igrid)%mesh%x(ixG^T,1:ndim))
       end do
       !$OMP END PARALLEL DO
    end if
    ...
    if (time_advance) then
       call getbc(global_time+dt,0.d0,ps,1,nwflux,gc_hydro)
       call getbc(global_time+dt,0.d0,ps,1,nmetric,gc_metric)
    else
       call getbc(global_time,0.d0,ps,1,nwflux,gc_hydro)
  ...
  subroutine coarsen_grid_siblings(igrid,ipe,child_igrid,child_ipe,active)
    use mod_global_parameters
    use mod_initialize, only: initial_condition
    ...
    {do ic^DB=1,2\}
       ...
       if (ipeFi==mype) then
          ...
          if (ipe==mype) then
             ...
             call coarsen_grid(ps(igridFi),ixG^LL,ixM^LL,ps(igrid),ixG^LL,ixCo^L,.true.,.true.)
             ! remove solution space of child
             !call dealloc_node(igridFi)
          else
             ixCoGmin^D=1;
             ...
             call coarsen_grid(ps(igridFi),ixG^LL,ixM^LL,psc(igridFi), &
                               ixCoG^L,ixCoM^L,.true.,.true.)

             ! fixme: the itag for metric variables is not well tested. However,
             ! this seems will not be a problem. Nevertheless, try to come up a
             ! better number or maybe combining w and metric is a good idea.
             !itag=ipeFi*max_blocks+igridFi
             itag=ipeFi+igridFi
             isend=isend+1
             call MPI_ISEND(psc(igridFi)%w,1,type_coarse_block,ipe,itag, &
                            icomm,sendrequest(isend),ierrmpi)
             itag=itag+max_blocks
             call MPI_ISEND(psc(igridFi)%metric%vars,1,type_coarse_block_m,ipe,itag, &
                            icomm,sendrequest_m(isend),ierrmpi)
             if(stagger_grid) then
               do idir=1,ndim
          ...
          if (ipe==mype) then
             ...
             itag=itag+max_blocks
             call MPI_IRECV(ps(igrid)%metric%vars,1,type_sub_block_m(ic^D),ipeFi,itag, &
                            icomm,recvrequest_m(irecv),ierrmpi)
             if(stagger_grid) then
               do idir=1,ndim
                 !itag_stg=(npe+ipeFi+1)*max_blocks+igridFi*(ndir-1+idir)
                 itag_stg=(npe+ipeFi+1)+igridFi*(ndir-1+idir)
