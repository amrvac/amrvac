...
contains
  ...
  subroutine prolong_2nd_stg(sCo,sFi,ixCo^Lin,ixFi^Lin,dxCo^D,xComin^D,dxFi^D,xFimin^D,ghost,fine_^Lin)
    ...
    double precision :: bfluxCo(sCo%mesh%ixGs^S,nws),bfluxFi(sFi%mesh%ixGs^S,nws)
    double precision :: slopes(sCo%mesh%ixGs^S,ndim)
    {^IFTHREED
    ...
    associate(wCos=>sCo%ws, wFis=>sFi%ws)
    ! Assemble general indices
    ixGsmin^D=sFi%mesh%ixGsmin^D;
    ixGsmax^D=sFi%mesh%ixGsmax^D;
    ...
    do idim1=1,ndim
      ...
      bfluxFi(ixFisE^S,idim1)=wFis(ixFisE^S,idim1)*sFi%mesh%surfaceC(ixFisE^S,idim1)
      {^IFTWOD
      idim2=1+mod(idim1,2)
      }
      {^IFTHREED
    ...
    do idim1=1,ndim
      ...
      bfluxCo(ixCosE^S,idim1)=wCos(ixCosE^S,idim1)*sCo%mesh%surfaceC(ixCosE^S,idim1)
    end do
    ...
    !       Toth-Roe
            alpha(ixCo^S,idim1)=(sCo%mesh%dx(ixCo^S,idim2)-sCo%mesh%dx(ixCo^S,idim3))/(sCo%mesh%dx(ixCo^S,idim2)+sCo%mesh%dx(ixCo^S,idim3))
        end do
      end do
    end do
    }
    ...
    do idim1=1,ndim
      ...
      where(sFi%mesh%surfaceC(ixFisC^S,idim1)/=zero)
        wFis(ixFisC^S,idim1)=bfluxFi(ixFisC^S,idim1)/sFi%mesh%surfaceC(ixFisC^S,idim1)
      elsewhere
