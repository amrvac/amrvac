...
subroutine prolong_grid(child_igrid,child_ipe,igrid,ipe)
  ...
  {do ic^DB=1,2\}
    ...
    !call prolong_1st(ps(igrid)%w,ixCo^L,ps(ichild)%w,ps(ichild)%mesh%x)
  {end do\}
...
subroutine prolong_2nd(sCo,ixCo^L,sFi,dxCo^D,xComin^D,dxFi^D,xFimin^D,igridCo,igridFi)
  use mod_physics, only: phys_to_primitive, phys_handle_small_values
  use mod_global_parameters
  ...
  ! refine metric first
  metric_vars: associate(wCo=>sCo%metric%vars, wFi=>sFi%metric%vars)
  ...
  {do ixCo^DB = ixCg^LIM^DB
     ...
     {do ix^DB=ixFi^DB,ixFi^DB+1 \}
        ...
        if(slab_uniform) then
          ...
          eta^D=(dble(ix^D-ixFi^D)-0.5d0)*(one-sFi%mesh%dvolume(ix^DD) &
                /sum(sFi%mesh%dvolume(ixFi^D:ixFi^D+1^D%ix^DD)))  \}
        end if
        wFi(ix^D,nwmin:nwmax) = wCo(ixCo^D,nwmin:nwmax) &
                              + {(slope(nwmin:nwmax,^D)*eta^D)+}
     {end do\}
  {end do\}

  deallocate(slope)
  end associate metric_vars

  hydro_vars: associate(wCo=>sCo%w, wFi=>sFi%w)
  ...
  if(fix_small_values) call phys_handle_small_values(sFi,ixG^LL,ixM^LL, 'prolong_2nd')

  deallocate(slope)
  end associate hydro_vars
