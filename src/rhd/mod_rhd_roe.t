...
contains
  ...
  !> rho -> rho, m -> v, e -> h
  subroutine rhd_average(wL,wR,x,ix^L,idim,wroe,workroe)
    ...
    call rhd_get_pthermal(wL,x,ixG^LL,ix^L, workroe(ixG^T, 2))

    wroe(ix^S,e_)    = (workroe(ix^S, 2)+wL(ix^S,e_))/wL(ix^S,rho_)

    call rhd_get_pthermal(wR,x,ixG^LL,ix^L, workroe(ixG^T, 2))
  ...
  subroutine rhd_get_eigenjump(wL,wR,wroe,x,ix^L,il,idim,smalla,a,jump,workroe)
    use mod_global_parameters
  ...
  subroutine geteigenjump2(wL,wR,wroe,x,ix^L,il,idim,smalla,a,jump,&
    ...
    if(il==1)then
       ...
       csound(ix^S)=(rhd_gamma-one)*(wroe(ix^S,e_) - kin_en(ix^S))
       ! Make sure that csound**2 is positive
       csound(ix^S)=max(rhd_gamma*smalldouble/wroe(ix^S,rho_),csound(ix^S))

       ! Calculate (pR-pL)/c**2
       ! To save memory we use tmp amnd tmp2 for pL and pR (rhd_get_pthermal is OK)
       call rhd_get_pthermal(wL,x,ixG^LL,ix^L,tmp)
       call rhd_get_pthermal(wR,x,ixG^LL,ix^L,tmp2)
    ...
    case('harten','powell')
       ...
       if (il == soundRW_) then
          ...
          tmp(ix^S)=wL(ix^S,mom(idim))/wL(ix^S,rho_)&
               + sqrt(rhd_gamma*tmp(ix^S)/wL(ix^S,rho_))
          call rhd_get_pthermal(wR,x,ixG^LL,ix^L,tmp2)
          tmp2(ix^S)=wR(ix^S,mom(idim))/wR(ix^S,rho_)&
               + sqrt(rhd_gamma*tmp2(ix^S)/wR(ix^S,rho_))
       else if (il == soundLW_) then
          call rhd_get_pthermal(wL,x,ixG^LL,ix^L,tmp)
          tmp(ix^S)=wL(ix^S,mom(idim))/wL(ix^S,rho_)&
               - sqrt(rhd_gamma*tmp(ix^S)/wL(ix^S,rho_))
  ...
  subroutine rhd_rtimes(q,wroe,ix^L,iw,il,idim,rq,workroe)
    use mod_global_parameters
  ...
  subroutine rhd_average_iso(wL,wR,x,ix^L,idim,wroe,workroe)
  ...
  subroutine rhd_get_eigenjump_iso(wL,wR,wroe,x,ix^L,il,idim,smalla,a,jump,workroe)
  ...
  subroutine geteigenjump2_iso(wL,wR,wroe,x,ix^L,il,idim,smalla,a,jump,csound)
    ...
    select case (typeaverage)
    case ('arithmetic')
       call rhd_get_pthermal(wroe,x,ixG^LL,ix^L,csound)
       csound(ix^S) = sqrt(rhd_gamma*csound(ix^S)/wroe(ix^S,rho_))
    ...
    case ('roe','default')
       ...
       where(abs(wL(ix^S,rho_)-wR(ix^S,rho_))<=qsmall*(wL(ix^S,rho_)+wR(ix^S,rho_)))
          csound(ix^S) = sqrt(rhd_gamma*csound(ix^S)/wroe(ix^S,rho_))
       elsewhere
          csound(ix^S) =  sqrt(rhd_gamma*(tmp2(ix^S)-tmp(ix^S))/&
               (wR(ix^S,rho_)-wL(ix^S,rho_)))
       end where
  ...
  subroutine rhd_rtimes_iso(q,wroe,ix^L,iw,il,idim,rq,workroe)
