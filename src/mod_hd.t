...
contains
  ...
  subroutine hd_activate
    ...
    if (hd_energy) then
       ! Characteristic
       soundRW_=1, soundLW_=2, entropW_=3, shearW0_=3      ! waves
       nworkroe=3
    end if

    if (hd_iso) then
       ! Characteristic
       soundRW_=1, soundLW_=2, shearW0_=2        ! waves
       nworkroe=1
    end if

    if (hd_energy) then
       gamma_=1, mu_=2, neqpar=2                     ! equation parameters
    end if
    if (hd_iso) then
       gamma_=1, adiab_=2, mu_=3, neqpar=3                     ! equation parameters
    end if
    nflag_=nw+1
