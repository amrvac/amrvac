    ...
    do iw=1,nw+nwauxio
      do ix=ixMlo1,ixMhi1
        ps_sub(jgrid)%w(ixMlo2:ixMhi2,ixMlo3:ixMhi3,iw) = &
             ...
             mesh(igrid)%dx(ix,ixMlo2:ixMhi2,ixMlo3:ixMhi3,1)
      end do
    end do
  end if
  ps_sub(jgrid)%x(ixMlo2:ixMhi2,ixMlo3:ixMhi3,1:ndim) = &
       mesh(igrid)%x(ixMlo1,ixMlo2:ixMhi2,ixMlo3:ixMhi3,1:ndim)
case (2)
  if(slab_uniform) then
    do ix=ixMlo2,ixMhi2
    ...
    do iw=1,nw+nwauxio
      do ix=ixMlo2,ixMhi2
         ps_sub(jgrid)%w(ixMlo1:ixMhi1,ixMlo3:ixMhi3,iw) = &
             ...
             mesh(igrid)%dx(ixMlo1:ixMhi1,ix,ixMlo3:ixMhi3,2)
      end do
    end do
  endif
  ps_sub(jgrid)%x(ixMlo1:ixMhi1,ixMlo3:ixMhi3,1:ndim) = &
       mesh(igrid)%x(ixMlo1:ixMhi1,ixMlo2,ixMlo3:ixMhi3,1:ndim) 
case (3)
  if(slab_uniform) then
    do ix=ixMlo3,ixMhi3
    ...
    do iw=1,nw+nwauxio
      do ix=ixMlo3,ixMhi3
         ps_sub(jgrid)%w(ixMlo1:ixMhi1,ixMlo2:ixMhi2,iw) = &
             ...
             mesh(igrid)%dx(ixMlo1:ixMhi1,ixMlo2:ixMhi2,ix,3)
      end do
    end do
  endif
  ps_sub(jgrid)%x(ixMlo1:ixMhi1,ixMlo2:ixMhi2,1:ndim) = &
       mesh(igrid)%x(ixMlo1:ixMhi1,ixMlo2:ixMhi2,ixMlo3,1:ndim) 
case default
      ...
      do ix=ixMlo1,ixMhi1
        ps_sub(jgrid)%w(ixMlo2:ixMhi2,iw) = &
             ...
             + wCC_TMP(ix,ixMlo2:ixMhi2,iw) * &
              mesh(igrid)%dx(ix,ixMlo2:ixMhi2,1)
      end do
    end do
  end if
  ps_sub(jgrid)%x(ixMlo2:ixMhi2,1:ndim) = &
       mesh(igrid)%x(ixMlo1,ixMlo2:ixMhi2,1:ndim)
case (2)
  if(slab_uniform) then
    do ix=ixMlo2,ixMhi2
    ...
    do iw=1,nw+nwauxio
      do ix=ixMlo2,ixMhi2
         ps_sub(jgrid)%w(ixMlo1:ixMhi1,iw) = &
              ...
              mesh(igrid)%dx(ixMlo1:ixMhi1,ix,2)
      end do
    end do
  end if
  ps_sub(jgrid)%x(ixMlo1:ixMhi1,1:ndim) = &
       mesh(igrid)%x(ixMlo1:ixMhi1,ixMlo2,1:ndim) 
case default
   call mpistop("slice direction not clear in collapse_subnode")
end select
}
...
if(slab_uniform) then
  ...
  do iw=1,nw+nwauxio
    do ix=ixMlo1,ixMhi1
      ps_sub(jgrid)%w(iw) = ps_sub(jgrid)%w(iw) + wCC_TMP(ix,iw) * mesh(igrid)%dx(ix,1)
    end do
  end do
end if
ps_sub(jgrid)%x(1:ndim) = mesh(igrid)%x(ixMlo1,1:ndim)
}
