...
contains
  ...
  subroutine read_par_files()
    use mod_global_parameters
    use mod_physics, only: physics_type, phys_wider_stencil, phys_extra_ghostcells
    ...
    character(len=std_len), dimension(:), allocatable :: &
         typeboundary_metric_min^D, typeboundary_metric_max^D
    ...
    namelist /filelist/ base_filename,restart_from_file, &
         ...
         convert,convert_type,saveprim,savewextra,usr_filename,&
    ...
    namelist /methodlist/ time_stepper,time_integrator, &
         ...
         PPM_extrema,PPM_flatcd,PPM_flatsh,&
    ...
    namelist /boundlist/ nghostcells,ghost_copy,&
         internalboundary, typeboundary_metric_^L, typeboundary_^L, save_physical_boundary
    ...
    allocate(typeboundary_metric_min^D(nmetric))
    allocate(typeboundary_metric_max^D(nmetric))
    typeboundary_metric_min^D = undefined
    typeboundary_metric_max^D = undefined
    }
    ...
    four_lvc = 0
    do iw=0,3; do i=0,3; do j=0,3; do k=0,3
       four_lvc(iw,i,j,k) = sgn(k-j) * sgn(k-i)  * sgn(k-iw) &
                           *sgn(j-i) * sgn(j-iw) * sgn(i-iw)
...
107    close(unitpar)
    ...
    ! fixme: split the options for riemann solvers and FV vs FD
    do level=1,nlevelshi
       ...
       case ('fd_tvdlf')
          flux_method(level)=fs_fd_tvdlf
       case ('fd_hll')
          flux_method(level)=fs_fd_hll
       case ('source')
    ...
    if(any(limiter(1:nlevelshi)=='ppm')) then
      ...
      if (stagger_grid .and. PPM_extrema) then
         phys_extra_ghostcells = 1
      end if
    end if
    ...
    nghostcells = nghostcells + phys_wider_stencil + phys_extra_ghostcells
    ...
    fv_n_interp = min(fv_n_interp, nghostcells * 2)
  ...
  subroutine snapshot_write_header(fh, offset_tree, offset_block, dataset_names, nw_vars)
    use mod_forest
    use mod_physics
    use mod_global_parameters
    use mod_slice, only: slicenext
    ...
    call MPI_FILE_WRITE(fh, [ xprobmin^D ], ndim, MPI_DOUBLE_PRECISION, st, er)
    call MPI_FILE_WRITE(fh, [ xprobmax^D ], ndim, MPI_DOUBLE_PRECISION, st, er)
    call MPI_FILE_WRITE(fh, [ domain_nx^D ], ndim, MPI_INTEGER, st, er)
    call MPI_FILE_WRITE(fh, [ block_nx^D ], ndim, MPI_INTEGER, st, er)
  ...
  subroutine snapshot_read_header(fh, offset_tree, offset_block)
    ...
    call MPI_FILE_READ(fh, ibuf(1), 1, MPI_INTEGER, st, er)
    nw_found=ibuf(1)
    if (nmetric+nw+nw_extra /= ibuf(1)) then
      write(*,*) "nmetric=",nmetric
      write(*,*) "nw=",nw
      write(*,*) "nw_extra=",nw_extra
      write(*,*) "nw found in restart file=",ibuf(1)
  ...
  subroutine block_shape_io(igrid, n_ghost, ixO^L, n_values)
    ...
    if(save_physical_boundary) then
      do idim=1,ndim
        ...
        if(mesh(igrid)%is_physical_boundary(2*idim-1)) n_ghost(idim)=nghostcells
        ! Include ghost cells on upper boundary
        if(mesh(igrid)%is_physical_boundary(2*idim)) n_ghost(ndim+idim)=nghostcells
      end do
    end if
  ...
  subroutine write_snapshot
    ...
    character(len=name_len), allocatable :: w_names(:)
    ...
    n_vars = nmetric + nw
    if (savewextra.and.nw_extra>0) n_vars = n_vars + nw_extra
    n_values = count_ix(ixG^LL) * n_vars
    if(stagger_grid) then
      n_values = n_values + count_ix(ixGs^LL) * nws
    end if
    allocate(w_buffer(n_values)) ! fixme: we don't need to deallocate them?
    ...
    if (mype==0) then
      ...
      allocate(w_names(n_vars))
      n_offset = 0
      w_names(n_offset+1:n_offset+nmetric) = metric_names(1:nmetric)
      n_offset = n_offset + nmetric
      w_names(n_offset+1:n_offset+nw) = prim_names(1:nw)
      n_offset = n_offset + nw
      if (savewextra.and.nw_extra>0) w_names(n_offset+1:n_offset+nw_extra) = wextra_names(1:nw_extra)
    ...
    do Morton_no=Morton_start(mype), Morton_stop(mype)
      ...
      ! store data to w_buffer
      w_buffer(n_offset+1:n_offset+n_values*nmetric) = pack(ps(igrid)%metric%vars(ixO^S, 1:nmetric), .true.)
      n_offset = n_offset + n_values*nmetric

      w_buffer(n_offset+1:n_offset+n_values*nw) = pack(ps(igrid)%w(ixO^S, 1:nw), .true.)
      n_offset = n_offset + n_values*nw

      if (savewextra.and.nw_extra>0) then
         w_buffer(n_offset+1:n_offset+n_values*nw_extra) = pack(ps(igrid)%wextra(ixO^S, 1:nw_extra), .true.)
         n_offset = n_offset + n_values*nw_extra
      end if
      ...
      if(stagger_grid) then
        ...
        w_buffer(n_offset+1:n_offset+n_values_stagger) = pack(ps(igrid)%ws(ixOs^S, 1:nws), .true.)
        n_offset = n_offset + n_values_stagger
      end if
      ...
      do ipe = 0, npe-1
        do Morton_no=Morton_start(ipe),Morton_stop(ipe)
          ...
          if (mype == ipe) then ! Root task
            ...
            !    call usr_transform_w(ixG^LL,ixM^LL,nw_found,w,ps(igrid)%mesh%x,ps(igrid)%w)
            !  else
            ...
            !  ps(igrid)%w(ixO^S,1:nw)=w(ixO^S,1:nw)
            !end if
            metric(igrid)%vars(ixO^S,1:nmetric)=w(ixO^S,1:nmetric)
            ps(igrid)%w(ixO^S,1:nw)=w(ixO^S,nmetric+1:nmetric+nw)
            if (nw_extra>0) &
               ps(igrid)%wextra(ixO^S,1:nw_extra)=w(ixO^S,nmetric+nw+1:nw_found)
          else
  ...
  subroutine printlog_default
    ...
    integer              :: amode, istatus(MPI_STATUS_SIZE), ierrmpi
  ...
  subroutine get_volume_average(power, mode, volume)
    ...
    do iigrid = 1, igridstail
       ...
       wsum(nw+1) = wsum(nw+1) + sum(ps(igrid)%mesh%dvolume(ixM^T))

       ! Compute the modes of the cell-centered variables, weighted by volume
       do iw = 1, nw
          wsum(iw) = wsum(iw) + &
               sum(ps(igrid)%mesh%dvolume(ixM^T)*ps(igrid)%w(ixM^T,iw)**power)
       end do
    end do
  ...
  subroutine get_volume_average_func(func, f_avg, volume)
    ...
    do iigrid = 1, igridstail
       ...
       ! Store total volume in last element
       wsum(2) = wsum(2) + sum(ps(igrid)%mesh%dvolume(ixM^T))

       ! Compute the modes of the cell-centered variables, weighted by volume
       {do i^D = ixMlo^D, ixMhi^D\}
       wsum(1) = wsum(1) + ps(igrid)%mesh%dvolume(i^D) * &
