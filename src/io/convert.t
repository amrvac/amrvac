   ...
   if (nwauxio > 0) then
      if (.not. associated(usr_aux_output)) then
         ...
         call usr_aux_output(ixG^LL,ixM^LL^LADD1, &
              ps1(igrid)%w,ps(igrid)%mesh%x,normconv)
      end if
   end if
end do
             ...
             select case(convert_type)
               case("oneblock")
                 write(qunit,fmt="(100(e14.6))") &
                  ps(igrid)%mesh%x(ix^D,1:ndim)*normconv(0),&
                  (ps1(igrid)%w(ix^D,iwrite(iw))*normconv(iwrite(iw)),iw=1,writenw)
               case("oneblockB")
                 write(qunit) real(ps(igrid)%mesh%x(ix^D,1:ndim)*normconv(0)),&
...
!.. MPI variables ..
integer           :: igrid_recv,ipe,ierrmpi
...
do Morton_no=Morton_start(mype),Morton_stop(mype)
  ...
  if (mype/=0)then
      ...
      call MPI_SEND(ps(igrid)%mesh%x,1,type_block_xcc_io, 0,itag,icomm,ierrmpi)
      itag=igrid
   ...
   {do ix^DB=ixMlo^DB,ixMhi^DB\}
      ...
      enddo
       write(qunit,fmt="(100(e14.6))") ps(igrid)%mesh%x(ix^D,1:ndim)&
