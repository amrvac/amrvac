...
contains
  ...
  subroutine CFG_read_file(cfg, filename)
    ...
    open(my_unit, FILE=trim(filename), status = "OLD", action="READ", err=998,&
        iostat=io_state)
    line_number = 0
    ...
    do
       ...
       if (equal_sign_ix == 0) then
          ...
          if (line(1:1) /= '[' .or. ix == 0) then
             write(err_string, *) "Cannot read line ", line_number, " from ",&
                 trim(filename)
...
999 close(my_unit, iostat=io_state)
  ...
  subroutine read_variable(var)
    ...
    call get_fields_string(var%stored_data, CFG_separators, CFG_max_array_size,&
  ...
  subroutine CFG_write(cfg_in, filename, hide_unused)
    ...
    do i = 1, cfg%num_vars
       ...
       if (category /= "") then
          ...
          write(myUnit, ADVANCE="NO", ERR=998,&
              FMT="(A)") "    " // trim(var_name) // " ="
       else
          write(myUnit, ERR=998, FMT="(A,A,A)") "# ",&
              trim(cfg%vars(i)%description), ":"
       ...
       case (CFG_string_type)
          do j = 1, cfg%vars(i)%var_size
             write(myUnit, ADVANCE="NO", ERR=998,&
                 FMT="(A)") " '" // trim(cfg%vars(i)%char_data(j)) // "'"
          end do
       case (CFG_logic_type)
          do j = 1, cfg%vars(i)%var_size
...
999 continue ! If there was an error, the routine will end here
  ...
  subroutine prepare_store_var(cfg, var_name, var_type, var_size, description,&
      ix, dynamic_size)
    type(CFG_t), intent(inout)    :: cfg
  ...
  subroutine add_real_array(cfg, var_name, real_data, comment, dynamic_size)
    ...
    call prepare_store_var(cfg, var_name, CFG_real_type, size(real_data),&
  ...
  subroutine add_int_array(cfg, var_name, int_data, comment, dynamic_size)
    ...
    call prepare_store_var(cfg, var_name, CFG_integer_type, size(int_data),&
        comment, ix, dynamic_size)
  ...
  subroutine add_string_array(cfg, var_name, char_data, comment, dynamic_size)
    ...
    call prepare_store_var(cfg, var_name, CFG_string_type, size(char_data),&
        comment, ix, dynamic_size)
  ...
  subroutine add_logic_array(cfg, var_name, logic_data, comment, dynamic_size)
    ...
    call prepare_store_var(cfg, var_name, CFG_logic_type, size(logic_data),&
        comment, ix, dynamic_size)
  ...
  subroutine get_real_array(cfg, var_name, real_data)
    ...
    call prepare_get_var(cfg, var_name, CFG_real_type, size(real_data), ix)
  ...
  subroutine get_int_array(cfg, var_name, int_data)
    ...
    call prepare_get_var(cfg, var_name, CFG_integer_type, size(int_data), ix)
  ...
  subroutine get_string_array(cfg, var_name, char_data)
    ...
    call prepare_get_var(cfg, var_name, CFG_string_type, size(char_data), ix)
  ...
  subroutine get_logic_array(cfg, var_name, logic_data)
    ...
    call prepare_get_var(cfg, var_name, CFG_logic_type, size(logic_data), ix)
  ...
  subroutine add_get_real_array(cfg, var_name, real_data, comment,&
      dynamic_size)
    type(CFG_t), intent(inout)   :: cfg
    character(len=*), intent(in) :: var_name, comment
    real(dp), intent(inout)      :: real_data(:)
  ...
  subroutine add_get_string_array(cfg, var_name, char_data, comment,&
      dynamic_size)
    type(CFG_t), intent(inout)      :: cfg
    character(len=*), intent(in)    :: var_name, comment
    character(len=*), intent(inout) :: char_data(:)
  ...
  subroutine add_get_logic_array(cfg, var_name, logic_data, comment,&
      dynamic_size)
    type(CFG_t), intent(inout)   :: cfg
    character(len=*), intent(in) :: var_name, comment
    logical, intent(inout)       :: logic_data(:)
  ...
  subroutine get_fields_string(line, delims, n_max, n_found, ixs_start,&
      ixs_end)
