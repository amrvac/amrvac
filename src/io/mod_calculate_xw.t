  ...
  subroutine calc_grid(qunit,igrid,xC,xCC,xC_TMP,xCC_TMP,wC_TMP,wCC_TMP,normconv,&
    ...
    if(nwauxio>0)then
      ...
      if (.not. associated(usr_aux_output)) then
         ...
         call usr_aux_output(ixG^LL,ixM^LL^LADD1,w,ps(igrid)%mesh%x,normconv)
      end if
    endif
    ...
    if(slab_uniform) then
       ...
       do iw=1,nw+nwauxio
         {do ix^DB=ixCmin^DB,ixCmax^DB\}
           wC(ix^D,iw)=sum(w(ix^D:ix^D+1,iw)*ps(igrid)%mesh%dvolume(ix^D:ix^D+1)) &
                    /sum(ps(igrid)%mesh%dvolume(ix^D:ix^D+1))
         {end do\}
       end do
    endif
...
!    end if
  ...
  subroutine calc_x(igrid,xC,xCC)
    ...
    xCC(ixM^T,:)=ps(igrid)%mesh%x(ixM^T,:)

    ! coordinates of cell corners
    ixCmin^D=ixMlo^D-1; ixCmax^D=ixMhi^D;
    if(slab_uniform)then
       do idims=1,ndim
         xC(ixC^S,idims)=ps(igrid)%mesh%x(ixC^S,idims)+0.5d0*dx(idims,level)
       end do
    else
       ! for any non-cartesian or stretched coordinate (allow multiple stretched directions)
       {do ix=ixCmin^D,ixCmax^D
         xC(ix^D%ixC^S,^D)=ps(igrid)%mesh%x(ix^D%ixC^S,^D)+0.5d0*ps(igrid)%mesh%dx(ix^D%ixC^S,^D)
       end do\}
    endif
