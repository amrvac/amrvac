...
!         call MPI_FILE_WRITE(slice_fh,neqpar+nspecialpar,1,MPI_INTEGER,status,ierrmpi)
  ...
  subroutine fill_subnode(igrid,active,jgrid,dir,xslice,normconv)
    ...
    {^IFONED
    ixslice = minloc(dabs(xslice-mesh(igrid)%x(:,dir)),1,mask(:))
    }
    {^IFTWOD
    select case (dir)
    case (1)
       ixslice = minloc(dabs(xslice-mesh(igrid)%x(:,ixMlo2,dir)),1,mask(:,ixMlo2))
    case (2)
       ixslice = minloc(dabs(xslice-mesh(igrid)%x(ixMlo1,:,dir)),1,mask(ixMlo1,:))
    case default
    ...
    select case (dir)
    case (1)
       ixslice = minloc(dabs(xslice-mesh(igrid)%x(:,ixMlo2,ixMlo3,dir)),1,mask(:,ixMlo2,ixMlo3))
    case (2)
       ixslice = minloc(dabs(xslice-mesh(igrid)%x(ixMlo1,:,ixMlo3,dir)),1,mask(ixMlo1,:,ixMlo3))
    case (3)
       ixslice = minloc(dabs(xslice-mesh(igrid)%x(ixMlo1,ixMlo2,:,dir)),1,mask(ixMlo1,ixMlo2,:))
    case default
       call mpistop("slice direction not clear in fill_subnode")
    end select
    }
  ...
  subroutine alloc_subnode(jgrid,dir,nwexpand)
    ...
    !allocate(ps_sub(jgrid)%cons(1:ncons))
