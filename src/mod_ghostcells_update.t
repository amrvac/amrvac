...
contains
  ...
  subroutine getbc(time,qdt,psb,nwstart,nwbc,req_diag)
    ...
    integer :: ixR^L, ixS^L
    integer :: i^D, n_i^D, ic^D, inc^D, n_inc^D, idir, istage
    ...
    !$acc parallel loop default(present) copyin(ixS_srl_^L,ixR_srl_^L) private(igrid,ineighbor,n_i^D,ixS^L,ixR^L,iw,ix^D) firstprivate(nwhead,nwtail)
    do iigrid=1,igridstail; igrid=igrids(iigrid);
      !$acc loop seq
      {do i^DB=-1,1\}
        if ((all([ i^D ] == 0))) cycle
