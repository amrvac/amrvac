...
contains
  ...
  subroutine twofl_average(wL,wR,x,ix^L,idim,wroe,workroe)
    use mod_global_parameters
  ...
  subroutine average2(wL,wR,x,ix^L,idim,wroe,cfast,cslow,afast,aslow,csound2,dp, &
    ...
    wroe(ix^S,rho_c_)=half*(wL(ix^S,rho_c_)+wR(ix^S,rho_c_))
    do idir=1,ndir
      wroe(ix^S,mom_c(idir))=half*(wL(ix^S,mom_c(idir))/wL(ix^S,rho_c_)+wR(ix^S,mom_c(idir))/wR(ix^S,rho_c_))
      wroe(ix^S,mag(idir))=half*(wL(ix^S,mag(idir))+wR(ix^S,mag(idir)))
    end do
    ! Use afast and aslow for pressures pL and pR
    call twofl_get_pthermal_c(wL,x,ixG^LL,ix^L,afast)
    call twofl_get_pthermal_c(wR,x,ixG^LL,ix^L,aslow)

    if(twofl_eq_energy/=0) then
      wroe(ix^S,e_c_)=half*(afast(ix^S)+aslow(ix^S))
      ! dp=pR-pL
    ...
    rhodv(ix^S)=wR(ix^S,mom_c(idim))-wL(ix^S,mom_c(idim))-&
         wroe(ix^S,mom_c(idim))*(wR(ix^S,rho_c_)-wL(ix^S,rho_c_))

    !Calculate csound2,cfast,cslow,alphafast and alphaslow

    ! get csound**2
    if(twofl_eq_energy/=0) then
      csound2(ix^S)=twofl_gamma*wroe(ix^S,e_c_)/wroe(ix^S,rho_c_)
    else
      csound2(ix^S)=twofl_gamma*twofl_adiab*wroe(ix^S,rho_c_)**(twofl_gamma-one)
    end if

    ! aa=B**2/rho+a**2
    cfast(ix^S)=sum(wroe(ix^S,mag(:))**2,dim=ndim+1)/wroe(ix^S,rho_c_)+csound2(ix^S)

    ! cs**2=0.5*(aa+dsqrt(aa**2-4*a**2*(b_i**2/rho)))
    cslow(ix^S)=half*(cfast(ix^S)-dsqrt(cfast(ix^S)**2-&
         4d0*csound2(ix^S)*wroe(ix^S,mag(idim))**2/wroe(ix^S,rho_c_)))
    ...
    ! rho -> dsqrt(rho)
    wroe(ix^S,rho_c_)=dsqrt(wroe(ix^S,rho_c_))
  ...
  subroutine twofl_get_eigenjump(wL,wR,wroe,x,ix^L,il,idim,smalla,a,jump,workroe)
    use mod_global_parameters
  ...
  subroutine geteigenjump2(wL,wR,wroe,x,ix^L,il,idim,smalla,a,jump, &
    ...
    if(il==fastRW_)then
       ...
       bdv(ix^S)=wroe(ix^S,mag(idir))* &
            (wR(ix^S,mom_c(idir))/wR(ix^S,rho_c_)-wL(ix^S,mom_c(idir))/wL(ix^S,rho_c_))
       if(ndir==3)bdv(ix^S)=bdv(ix^S)+wroe(ix^S,mag(jdir))* &
            (wR(ix^S,mom_c(jdir))/wR(ix^S,rho_c_)-wL(ix^S,mom_c(jdir))/wL(ix^S,rho_c_))
       bdv(ix^S)=bdv(ix^S)*sign(wroe(ix^S,rho_c_)**2,wroe(ix^S,mag(idim)))
       ...
       bdb(ix^S)=bdb(ix^S)*dsqrt(csound2(ix^S))*wroe(ix^S,rho_c_)
    endif
    ...
    if(il==alfvRW_)then
       ...
       bdv(ix^S)=bdv(ix^S)*half*wroe(ix^S,rho_c_)**2
       bdb(ix^S)=bdb(ix^S)*half*sign(wroe(ix^S,rho_c_),wroe(ix^S,mag(idim)))
    endif

    select case(il)
    case(fastRW_)
       a(ix^S)=wroe(ix^S,mom_c(idim))+cfast(ix^S)
    ...
    case(fastLW_)
       ...
       jump(ix^S)=half/csound2(ix^S)*(&
            ...
            +aslow(ix^S)*(+cslow(ix^S)*bdv(ix^S)+bdb(ix^S)))
    case(slowRW_)
       a(ix^S)=wroe(ix^S,mom_c(idim))+cslow(ix^S)
       jump(ix^S)=half/csound2(ix^S)*(&
    ...
    case(slowLW_)
       ...
       jump(ix^S)=half/csound2(ix^S)*(&
            ...
            +afast(ix^S)*(-cfast(ix^S)*bdv(ix^S)-bdb(ix^S)))
    case(entroW_)
       a(ix^S)=wroe(ix^S,mom_c(idim))
       jump(ix^S)=wR(ix^S,rho_c_)-wL(ix^S,rho_c_)-dp(ix^S)/csound2(ix^S)
    case(diverW_)
       if(divbwave)then
          a(ix^S)=wroe(ix^S,mom_c(idim))
    ...
    case(alfvRW_)
       a(ix^S)=wroe(ix^S,mom_c(idim))+dabs(wroe(ix^S,mag(idim)))/wroe(ix^S,rho_c_)
    ...
    case('harten','powell', 'ratio')
       ...
       aL(ix^S)= wL(ix^S,mom_c(idim))/wL(ix^S,rho_c_)
       aR(ix^S)= wR(ix^S,mom_c(idim))/wR(ix^S,rho_c_)
       ...
       case(fastRW_)
          ...
          call twofl_get_csound2_c_from_conserved(wL,x,ixG^LL,ix^L,cs2L)
          call twofl_get_csound2_c_from_conserved(wR,x,ixG^LL,ix^L,cs2R)
          cs2ca2L(ix^S)=cs2L(ix^S)+sum(wL(ix^S,mag(:))**2,dim=ndim+1)/wL(ix^S,rho_c_)
          cs2ca2R(ix^S)=cs2R(ix^S)+sum(wR(ix^S,mag(:))**2,dim=ndim+1)/wR(ix^S,rho_c_)
          ! Save the discriminants into cs2L and cs2R
          cs2L(ix^S)=&
               dsqrt(cs2ca2L(ix^S)**2-4d0*cs2L(ix^S)*wL(ix^S,mag(idim))**2/wL(ix^S,rho_c_))
          cs2R(ix^S)=&
               dsqrt(cs2ca2R(ix^S)**2-4d0*cs2R(ix^S)*wR(ix^S,mag(idim))**2/wR(ix^S,rho_c_))
       ...
       case(alfvRW_)
          ...
          cs2ca2L(ix^S)=dabs(wL(ix^S,mag(idim)))/dsqrt(wL(ix^S,rho_c_))
          cs2ca2R(ix^S)=dabs(wR(ix^S,mag(idim)))/dsqrt(wR(ix^S,rho_c_))
  ...
  subroutine twofl_rtimes(q,w,ix^L,iw,il,idim,rq,workroe)
    use mod_global_parameters
  ...
  subroutine rtimes2(q,wroe,ix^L,iw,il,idim,rq, &
    ...
    if(iw == rho_c_) then
      ...
      if(il==fastRW_)then
        ...
        v2a2(ix^S)=half*sum(wroe(ix^S,mom_c(:))**2,dim=ndim+1)+ &
             (two-twofl_gamma)/(twofl_gamma-one)*csound2(ix^S)
        ! Store sgn(bx)*(betay*vy+betaz*vz) in bv
        bv(ix^S)=wroe(ix^S,mag(idir))*wroe(ix^S,mom_c(idir))
        if(ndir==3)bv(ix^S)=bv(ix^S)+wroe(ix^S,mag(jdir))*wroe(ix^S,mom_c(jdir))
        bv(ix^S)=bv(ix^S)*sign(one,wroe(ix^S,mag(idim)))
      else if(il==alfvRW_)then
        !Store betaz*vy-betay*vz in bv
        bv(ix^S)=(wroe(ix^S,mag(jdir))*wroe(ix^S,mom_c(idir))-&
             wroe(ix^S,mag(idir))*wroe(ix^S,mom_c(jdir)))
      endif

      select case(il)
      case(fastRW_)
        rq(ix^S)=q(ix^S)*(-aslow(ix^S)*cslow(ix^S)*bv(ix^S)+afast(ix^S)*&
             (v2a2(ix^S)+cfast(ix^S)*(cfast(ix^S)+wroe(ix^S,mom_c(idim)))))
      case(fastLW_)
        rq(ix^S)=q(ix^S)*(+aslow(ix^S)*cslow(ix^S)*bv(ix^S)+afast(ix^S)*&
             (v2a2(ix^S)+cfast(ix^S)*(cfast(ix^S)-wroe(ix^S,mom_c(idim)))))
      case(slowRW_)
        rq(ix^S)=q(ix^S)*(+afast(ix^S)*cfast(ix^S)*bv(ix^S)+aslow(ix^S)*&
             (v2a2(ix^S)+cslow(ix^S)*(cslow(ix^S)+wroe(ix^S,mom_c(idim)))))
      case(slowLW_)
        rq(ix^S)=q(ix^S)*(-afast(ix^S)*cfast(ix^S)*bv(ix^S)+aslow(ix^S)*&
             (v2a2(ix^S)+cslow(ix^S)*(cslow(ix^S)-wroe(ix^S,mom_c(idim)))))
      case(entroW_)
        rq(ix^S)= q(ix^S)*half*sum(wroe(ix^S,mom_c(:))**2,dim=ndim+1)
      case(diverW_)
        if(divbwave)then
      ...
      if(iw==mom_c(idim))then
        ...
        case(fastRW_)
          rq(ix^S)=q(ix^S)*(afast(ix^S)*wroe(ix^S,iw)-aslow(ix^S)*&
               cslow(ix^S)*wroe(ix^S,mag(1)-mom_c(1)+iw)*sign(one,wroe(ix^S,mag(idim))))
        case(fastLW_)
          rq(ix^S)=q(ix^S)*(afast(ix^S)*wroe(ix^S,iw)+aslow(ix^S)*&
               cslow(ix^S)*wroe(ix^S,mag(1)-mom_c(1)+iw)*sign(one,wroe(ix^S,mag(idim))))
        case(slowRW_)
          rq(ix^S)=q(ix^S)*(aslow(ix^S)*wroe(ix^S,iw)+afast(ix^S)*&
               cfast(ix^S)*wroe(ix^S,mag(1)-mom_c(1)+iw)*sign(one,wroe(ix^S,mag(idim))))
        case(slowLW_)
          rq(ix^S)=q(ix^S)*(aslow(ix^S)*wroe(ix^S,iw)-afast(ix^S)*&
               cfast(ix^S)*wroe(ix^S,mag(1)-mom_c(1)+iw)*sign(one,wroe(ix^S,mag(idim))))
        case(entroW_)
      ...
      if(iw==mag(idim))then
        ...
        case(alfvRW_,alfvLW_)
          if(iw==mag(idir))then
            rq(ix^S)=-q(ix^S)*wroe(ix^S,mag(jdir))&
                 /sign(wroe(ix^S,rho_c_),wroe(ix^S,mag(idim)))
          else
            rq(ix^S)=+q(ix^S)*wroe(ix^S,mag(idir))&
                 /sign(wroe(ix^S,rho_c_),wroe(ix^S,mag(idim)))
          end if
