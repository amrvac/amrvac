...
w(ix^S,b1_)=tmp(ix^S)
!where(rval(ix^S)<one) 
!  w(ix^S,b1_)=cval*xJ1root*dsin(xtheta(ix^S))*dcos(xtheta(ix^S))*(DJ1vals(ix^S)-J1vals(ix^S)/(xJ1root*rval(ix^S)))
!elsewhere
!  w(ix^S,b1_)=two*x(ix^S,1)*x(ix^S,2)/rval(ix^S)**4
!endwhere
! compute dpsi0/dx
idims=1
select case(typegrad)
...
w(ix^S,b2_)=-tmp(ix^S)
!where(rval(ix^S)<one) 
!  w(ix^S,b2_)=cval*xJ1root*dsin(xtheta(ix^S))**2*(DJ1vals(ix^S)-J1vals(ix^S)/(xJ1root*rval(ix^S))) &
!              -cval*xJ1root*DJ1vals(ix^S)
!elsewhere
!  w(ix^S,b2_)=-one+(x(ix^S,2)**2-x(ix^S,1)**2)/rval(ix^S)**4
!endwhere
...
do iigrid=1,igridstail; igrid=igrids(iigrid);
   ...
   wmean(b1_)=wmean(b1_)+half*sum(dvolume(ixM^T)* &
       (pw(igrid)%w(ixM^T,b1_)**2+pw(igrid)%w(ixM^T,b2_)**2))
