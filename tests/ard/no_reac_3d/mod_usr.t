...
contains
  ...
  subroutine no_reac_init(ixG^L,ix^L,w,x)
    ...
    double precision                :: x1, x2, x3, urand(ix^S)
    double precision                :: l1, l2, l3, dist2(ix^S)
    logical                         :: mymask(ix^S)

    x1 = xprobmin1 + 0.5d0 * (xprobmax1 - xprobmin1)
    x2 = xprobmin2 + 0.5d0 * (xprobmax2 - xprobmin2)
    x3 = xprobmin3 + 0.5d0 * (xprobmax3 - xprobmin3)
    l1 = xprobmax1 - xprobmin1
    l2 = xprobmax2 - xprobmin2
    l3 = xprobmax3 - xprobmin3
    ...
    case (1)
       ...
       where (abs(x(ix^S, 1) - x1) < 0.1d0 * l1 .and. &
            abs(x(ix^S, 2) - x2) < 0.1d0 * l2 .and. &
            abs(x(ix^S, 3) - x3) < 0.1d0 * l3)
