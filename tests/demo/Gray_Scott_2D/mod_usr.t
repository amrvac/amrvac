...
contains
  ...
  subroutine gray_scott_init(ixG^L,ix^L,w,x)
    ...
    case (5)
       ! sinusoidal pattern in the center of the domain
      where (x(ix^S, 1) .ge. 1.0 .and.  x(ix^S, 1) .le. 1.5 &
        .and. x(ix^S, 2) .ge. 1.0 .and.  x(ix^S, 2) .le. 1.5) 
          w(ix^S,v_) = 0.25d0 * (sin(4.0*dpi*x(ix^S, 1)))**2 * (sin(4.0*dpi*x(ix^S, 2)))**2 
      elsewhere
          w(ix^S,v_) = 0d0
      endwhere
      w(ix^S,u_) = 1d0 - 2d0*w(ix^S,v_) 
