...
do iigrid=1,igridstail; igrid=igrids(iigrid);
   pwold(igrid)%w(ixG^T,1:nwflux)=pw(igrid)%w(ixG^T,1:nwflux)
end do
...
! opedit: Just advance the active grids: 
     ...
     call MPI_ALLREDUCE(acc_mass_mype,acc_mass_global,1,&
           MPI_DOUBLE_PRECISION,MPI_SUM,icomm,ierrmpi)
     if (mype == 0) then
        acc_mass_tot=acc_mass_tot+acc_mass_global
        open(25,file="massAcc", access='append',status='unknown')
        write(25,'(3(e13.5))') qt,acc_mass_global,acc_mass_tot
	 close(25)
     end if
