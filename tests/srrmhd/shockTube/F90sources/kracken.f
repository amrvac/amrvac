...
!
      ...
      if (ios /= 0 )then
         valu8=0.0_k_dbl
         call send_message&
            ("*string_to_real* - cannot produce number from this string")
...
!=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
      if((currnt=="-".and.prev==" ".and.delmt == "off".and.index&
         ("0123456789.",forwrd) == 0).or.ipoint > islen)then
...
!     report that no room is left and set error flag and error message.
      ...
      if(ix2(ic) /= " ")then
        call send_message&
           ("*add_string* no room left to add more string variable names")
        ier=-1
