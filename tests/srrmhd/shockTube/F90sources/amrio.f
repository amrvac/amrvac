...
namelist /boundlist/  dixB,typeB,typeghostfill,typegridfill,ratebdflux,&
   internalboundary
namelist /amrlist/    mxnest,nbufferx1,tol,tolratio,errorestimate,&
    amr_wavefilter,nxlone1,dxlone1,iprob,xprobmin1,xprobmax1, skipfinestep,&
   wflags,flags,restrictprimitive,prolongprimitive,coarsenprimitive,&
    typeprolonglimit, amrentropy,logflag,tfixgrid,itfixgrid,ditregrid
...
if(dxlone1<smalldouble)then
   write(unitterm,*)'Wrong value(s) for level one dx:',dxlone1
...
if (mype.eq.0 .and. snapshotini.eq.-1 .and.  floor(dble(nxlone1)&
   /(dble(ixGhi1)-2.0d0*dble(dixB)))  .lt. npe) then
   call mpistop&
...
do Morton_no=Morton_start(mype),Morton_stop(mype)
   ...
   if (nwaux>0) then
      ...
      call getaux(.true.,pw(igrid)%w,px(igrid)%x,ixGlo1,ixGhi1,ixMlo1&
         -1,ixMhi1+1,"write_snapshot")
   endif
   iwrite=iwrite+1
 ...
 if(npe>1)then
  ...
  do ipe =1, npe-1
   do Morton_no=Morton_start(ipe),Morton_stop(ipe)
     ...
     allocate(pwio(igrid_recv(inrecv))%w(ixGlo1:ixGhi1,1:nw))
...
if (mype /= 0) then
 ...
 do Morton_no=Morton_start(0),Morton_stop(0)
   ...
   write(unitsnapshot) pw(igrid)%w(ixMlo1:ixMhi1,1:nw)
 end do
 ! write data communicated from other processors
 if(npe>1)then
  nrecv=(Morton_stop(npe-1)-Morton_start(1)+1)
  ...
  do ipe =1, npe-1
   do Morton_no=Morton_start(ipe),Morton_stop(ipe)
     ...
     write(unitsnapshot) pwio(igrid_recv(inrecv))%w(ixMlo1:ixMhi1,1:nw)
     deallocate(pwio(igrid_recv(inrecv))%w)
   end do
  end do
  deallocate(igrecvstatus,iorecvstatus,ioastatus,igrid_recv)
...
integer :: levmaxini, ndimini, ndirini, nwini, neqparini, nxini1
...
if (mype==0) then
 ...
 if (ixGhi1/=nxini1+2*dixB) then
    write(*,*) "Error: reset resolution to ",nxini1+2*dixB
    call mpistop("change with setamrvac")
 end if
 neqparini=min(neqparini,neqpar+nspecialpar)
...
double precision :: dvolume(ixGlo1:ixGhi1), volumeflat(1:nlevelshi)
integer          :: numlevels, nx1, nc, ncells, dit
...
do iigrid=1,igridstail; igrid=igrids(iigrid);
   ...
   if (slab) then
      dvolume(ixMlo1:ixMhi1)=rnode(rpdx1_,igrid)
   else
      dvolume(ixMlo1:ixMhi1)=pgeo(igrid)%dvolume(ixMlo1:ixMhi1)
      volume(level)=volume(level)+sum(dvolume(ixMlo1:ixMhi1))
   end if
   do iw=1,nw
      wmean(iw)=wmean(iw)+sum(dvolume(ixMlo1:ixMhi1)*pw(igrid)%w&
         (ixMlo1:ixMhi1,iw))
   end do
end do
