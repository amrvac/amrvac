subroutine con2prim(lfac,xi,d,s1,s2,s3,tau,b1,b2,b3,e1,e2,e3,ierror)
...
double precision:: d,s1,s2,s3,tau,b1,b2,b3,e1,e2,e3
integer         :: ierror
      
double precision:: ssqr,bsqr,esqr,edotb,sdotecrossb,ecrossb2,ecrossb(1:3),&
   e(1:3),b(1:3)
...
double precision:: fv,dfv,v1,v2,v3,pressure
...
sdotecrossb = s1*ecrossb(1)+s2*ecrossb(2)+s3*ecrossb(3)
...
if(bsqr<=limitvalue .and. esqr<=limitvalue)then
  call con2primHydro(lfac,xi,d,s1,s2,s3,tau,ierror)
  return 
end if ! Hydro
...
if(fl>zero)then
  ...
  print*,'iteration  = ',it
  print *,'xi1=',xi1,' vs dplus=',dplus,'dplus*lfacl=',dplus*lfacl,'lfacl=',lfacl,'lfacmax=',lfacmax
...
LoopxiMax : do ni=1,maxitnr
    ...
    call funcd(xi2,fh,df,lfach,d,tau,ssqr,bsqr,esqr,ecrossb2,sdotecrossb,&
       ierror)
    if(ierror /=0)return
...
LoopNRRMHD :  do ni=1,maxitnr 
      ...
      call funcd(xii,f,df,lfaci,d,tau,ssqr,bsqr,esqr,ecrossb2,sdotecrossb,&
         ierror) 
      if(ierror /=0)return
      ...
      if((er<tolernr*er1).or.(dabs(f/df)<absaccnr))then
            call funcd(xii,f,df,lfaci,d,tau,ssqr,bsqr,esqr,ecrossb2,&
               sdotecrossb,ierror) 
            tests=(xii>=dplus*lfaci.and.lfaci<=lfacmax)
            if(tests)then
...
!=============================================================================
subroutine con2primHydro(lfac,xi,d,s1,s2,s3,tau,ierror)
!use ieee_arithmetic
include 'amrvacdef.f'
...
double precision :: d,s1,s2,s3,tau
integer          :: ierror

integer          :: ni,niiter
double precision :: pcurrent,pnew,pL,pR
double precision :: er,er1,ff,df,dp,v1,v2,v3
...
! ierror=4 : final v2=1 hence problem as lfac=1/0
...
sqrs=s1**2.0d0+s2**2.0d0+s3**2.0d0
...
LoopNR:  do ni=1,maxitnr
     ...
     if(xicurrent<smallxi) then
       ...
       print *,'values for d,s,tau,s2:',d,s1,s2,s3,tau,sqrs
       ierror=3 
       return
     endif
...
v2=s2/xicurrent
v3=s3/xicurrent
     lfac2inv=one - (v1**2.0d0+v2**2.0d0+v3**2.0d0)
     if(lfac2inv>zero) then
       ...
       print *,'iteration number:',ni
       print *,'values for d,s,tau,s2:',d,s1,s2,s3,tau,sqrs
       print *,'values for v,xi:',v1,v2,v3,xicurrent
       ierror=4
       return
     endif
     ...
     call FuncEnthalpy(pcurrent,lfac2inv,d,s1,s2,s3,tau,sqrs,xicurrent,&
...
v3=s3/xicurrent
	   ...
	   call Bisection_Enthalpy(pcurrent,lfac2inv,d,s1,s2,s3,&
...
v3=s3/xicurrent
	   ...
	   !== ZM calculation done using the EOS ==!
	   call Bisection_Enthalpy(pnew,lfac2inv,d,s1,s2,s3,&
                                   tau,sqrs,xicurrent,h,ierror)
    	   Nff=-xicurrent*lfac2inv + h 
 	   !=======================================!
	   !==== Iterate ====!
...
!=============================================================================
subroutine funcd(xi,F,dF,lfac,d,tau,ssqr,bsqr,esqr,ecrossb2,sdotecrossb,&
   ierror)
