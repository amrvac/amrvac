...
L     = 2.d35              ! erg s^-1; Spin down luminosity
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
eqpar(ri_)    = 0.04d0
eqpar(rhoe_)  = Ldash/4.0d0/dpi/eqpar(vext_)**2/eqpar(ds_)**2  ! ambient density if standoff distance at ds
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
normvar(rho0_)   = one!UNIT_DENSITY
normvar(rho1_)   = one!UNIT_DENSITY
normvar(n0_)     = one!UNIT_DENSITY
normvar(n_)      = one!UNIT_DENSITY
}
...
if (mype == 0) then
   ...
   print*, 'Velocity of ejecta: ', eqpar(vext_), 'that is', eqpar(vext_)*UNIT_VELOCITY, 'cm/s'
...
{^IFTWOD
      ...
      w(ixG^S,pp_)     = (eqpar(vext_)/eqpar(Mext_))**2 * eqpar(rhoe_)/eqpar(gamma_)
      w(ixG^S,flg_)    = zero
{#IFDEF EPSINF
...
call get_lfacpw(ixI^L,ixO^L,w,x,lfac_pw)
...
double precision, dimension(ixG^T) :: lfac_pw, delta, lfac_save
logical           :: patchw(ixG^T)
integer           :: idirmin
!----------------------------------------------------------------------------
...
! Obtain pw Lorentz factor:
call get_lfacpw(ixG^L,ixG^L,w,x,lfac_pw)
...
{#IFDEF EPSINF
! reset rho1 to floor:
where (w(ixO^S,Drho1_)<w(ixO^S,lfac_)*eqpar(rho1e_) .or. w(ixO^S,Drho1_) .ne. w(ixO^S,Drho1_) &
     .or. w(ixO^S,Drho1_)*zero .ne. zero)
   w(ixO^S,Drho1_)=w(ixO^S,lfac_)*eqpar(rho1e_)
end where
! reset rho0 to floor:
where (w(ixO^S,Drho0_)<w(ixO^S,Drho1_)*eqpar(rho0floor_) .or. w(ixO^S,Drho0_) .ne. w(ixO^S,Drho0_) &
     .or. w(ixO^S,Drho0_)*zero .ne. zero)
   w(ixO^S,Drho0_)=w(ixO^S,Drho1_)*eqpar(rho0floor_)
end where
! reset n to floor:
where (w(ixO^S,Dn_)<w(ixO^S,lfac_)*eqpar(rho1e_) .or. w(ixO^S,Dn_) .ne. w(ixO^S,Dn_) &
     .or. w(ixO^S,Dn_)*zero .ne. zero)
   w(ixO^S,Dn_)=w(ixO^S,lfac_)*eqpar(rho1e_)
end where
! reset n0 to floor:
where (w(ixO^S,Dn0_)<w(ixO^S,Dn_)*eqpar(rho0floor_) .or. w(ixO^S,Dn0_) .ne. w(ixO^S,Dn0_) &
     .or. w(ixO^S,Dn0_)*zero .ne. zero)
...
{#IFDEF GLM
   ...
   if (iB .eq. 3 .and. typeaxial .eq. 'cylindrical') then
      ! low pressure outflow.
      do ix=ixOmin2,ixOmax2
         w(ixOmin1:ixOmax1,ix,1:nwflux+nwaux) = w(ixOmin1:ixOmax1,ixOmax2+1,1:nwflux+nwaux)
      end do
      call primitive(ixI^L,ixO^L,w,x)
!      w(ixO^S,rho_)    = eqpar(rhoe_)
      w(ixO^S,pp_)     = (eqpar(vext_)/eqpar(Mext_))**2 * eqpar(rhoe_)/eqpar(gamma_)
...
{^IFTWOD integer, parameter :: nmin=50, nmax=300}
!-----------------------------------------------------------------------------
...
call MPI_ALLREDUCE(MPI_IN_PLACE,isum_send,numlevels,MPI_INTEGER, MPI_SUM,icomm,&
   ierrmpi)
   nleafs_level(levmin:levmax)=isum_send(1:numlevels)
...
! Mask out bubble volume:
   call get_lfacpw(ixG^LL,ixM^LL,pw(igrid)%w,px(igrid)%x,lfac_pw)
...
!=============================================================================
subroutine get_lfacpw(ixI^L,ixO^L,w,x,lfac_pw)
      ...
      double precision, intent(in)    :: x(ixI^S,1:ndim)
      double precision, intent(out)   :: lfac_pw(ixG^T)

 lfac_pw(ixO^S) = eqpar(lor_)
 
end subroutine get_lfacpw
!=============================================================================
subroutine get_delta(ixI^L,ixO^L,w,x,delta)
      ...
      double precision, intent(in)    :: w(ixI^S,1:nw)
      double precision, intent(in)    :: x(ixI^S,1:ndim)
      double precision, intent(out)   :: delta(ixG^T)
! ..local..
...
{^IFTWOD
   ...
   theta(ixI^S)    = x(ixI^S,2)
    ...
    where (theta(ixO^S) .le. dpi/2.0d0-eqpar(eta_)&
    .or. theta(ixO^S) .ge. dpi/2.0d0+eqpar(eta_))
        delta(ixO^S) = one
        elsewhere
    delta(ixO^S) = dabs(2.0d0/dpi*acos(-1.0d0/tan(theta(ixO^S))*1.0d0/tan(eqpar(eta_)) )-1.0d0)**2
    end where
...
subroutine get_sigma(ixI^L,ixO^L,w,x,sigma)
      ...
      double precision, intent(in)    :: w(ixI^S,1:nw)
      double precision, intent(in)    :: x(ixI^S,1:ndim)
      double precision, intent(out)   :: sigma(ixG^T)
! ..local..
